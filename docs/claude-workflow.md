...
| **Hooks** | Es gibt derzeit keine Regel, die deterministisch bei jedem Tool-Aufruf erzwungen werden muss. Sinnvoll würde ein `PreToolUse`-Hook, sobald das Xcode-Projekt existiert und etwa Schreibzugriffe auf `project.pbxproj` oder ein `swift-format`-Lauf nach jedem Edit abgesichert werden sollen. Zu bewerten in Phase 0. |
