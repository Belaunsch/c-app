...
Beleg:     zwei Primärquellen, dazu die Gegenprobe auf dem Gerät
Folge:     Nur NSMicrophoneUsageDescription ist gesetzt.
```
...
- `NSSpeechRecognitionUsageDescription` ist **belegt entbehrlich** und wird
  **nicht gesetzt**. Hier stand bis Phase 10 die Absicht, ihn „aus Vorsicht"
  trotzdem zu setzen; umgesetzt wurde das nie, und die Gegenprobe hat die
  Vorsicht erledigt: Phase 9 lief am 2026-09-11 auf dem iPhone 16 Pro
  vollständig **ohne** den Schlüssel — Vorbereitung, Mikrofonfreigabe,
  Aufnahme und Erkennung. Kein Absturz, kein Dialog, keine Fehlermeldung.
  Ein Schlüssel, der einen Zweck nur behauptet, ist kein billiges Extra,
  sondern eine falsche Aussage im Berechtigungsdialog.
...
| Schlüssel | Gesetzt? | Grund | Ab Phase |
| --- | --- | --- | --- |
| `NSMicrophoneUsageDescription` | **ja** | Aufnahme für die Spracherkennung | 9 |
| `NSSpeechRecognitionUsageDescription` | **nein** | **Belegt entbehrlich** für den `SpeechAnalyzer`-Pfad (Q4, §7) und auf dem Gerät gegengeprüft: Phase 9 lief ohne ihn vollständig durch | — |

Das ist der geprüfte Projektzustand, nicht die Absicht: `project.pbxproj`
setzt in beiden Konfigurationen ausschließlich
`INFOPLIST_KEY_NSMicrophoneUsageDescription`. Nachgesehen wurde nicht im
Projekt, sondern in der **gebauten** `Info.plist` des Release-Produkts:
`NSSpeechRecognitionUsageDescription` existiert dort nicht.

**Nebenbefund aus Phase 10 — der Launch-Screen-Hintergrund lässt sich über
`INFOPLIST_KEY_*` nicht setzen.** Ein Versuch, die Startfläche über
`INFOPLIST_KEY_UILaunchScreen_BackgroundColor` und danach über
`INFOPLIST_KEY_UILaunchScreen_UIColorName` einzufärben, blieb in beiden
Schreibweisen **wirkungslos**: Die gebaute `Info.plist` enthielt in allen drei
Varianten — mit der einen Einstellung, mit der anderen und ganz ohne —
identisch

```xml
<key>UILaunchScreen</key><dict><key>UILaunchScreen</key><dict/></dict>
```

Eine Warnung gibt es dabei nicht; die Einstellung wird stillschweigend
verworfen. Gegenprobe im selben Build: Für `UIApplicationSceneManifest`
erzeugt derselbe Generator sehr wohl Unterschlüssel, das Verfahren als solches
funktioniert also. Und in der gesamten Xcode-Installation existiert nur ein
einziger passender Einstellungsname, nämlich
`INFOPLIST_KEY_UILaunchScreen_Generation`. Für weitere Unterschlüssel bräuchte
es eine eigene `INFOPLIST_FILE`.

**Entschieden wurde dagegen**, und zwar nicht nur aus Bequemlichkeit: Der
leere `UILaunchScreen`-Eintrag ergibt eine Startfläche in der
Systemhintergrundfarbe, die von selbst hell und dunkel kann. Apples eigene
Empfehlung ist, dass der Launch-Screen dem **ersten Bildschirm** ähnelt und
kein Splash ist — der erste Bildschirm dieser App ist eine Liste auf
Systemhintergrund. Eine zinnoberrote Fläche davor wäre ein Blitz, der zur
weißen Liste springt. Das Rot bleibt da, wo es hingehört: im App-Icon. Für Übersetzung, TTS, Pinyin und
SwiftData sind keine Berechtigungen nötig. Der eine Schlüssel wurde erst in
Phase 9 gesetzt — vorher braucht die App ihn nicht, und ein unnötiger
Berechtigungsdialog wäre ein Rückschritt.

**Kein Privacy-Manifest, und das ist eine Entscheidung (Phase 13).** Eine
`PrivacyInfo.xcprivacy` gibt es nicht, weil die App keine „required reason
API" benutzt. Die einzige, die in Reichweite gekommen wäre, ist
`UITextInputMode.activeInputModes` — Apple führt sie unter
`NSPrivacyAccessedAPICategoryActiveKeyboards` mit dem zulässigen Grund
`54BD.1` („to present the correct customized user interface … may not be sent
off-device"). Sie würde gebraucht, um zu erkennen, ob überhaupt eine
chinesische Tastatur aktiv ist. **Genau deshalb wird sie nicht gelesen:** Der
Nutzen wäre ein Hinweissatz, die Kosten ein Manifest plus eine
Fingerprinting-nahe API in einer App, die sonst nichts über das Gerät liest
(harte Regel 8). Details in Q12.
...
| Q4 | ~~Braucht `SpeechAnalyzer` `NSSpeechRecognitionUsageDescription`?~~ **Beantwortet am 2026-09-11 aus zwei Primärquellen: nein** — der Schlüssel ist an den Serverpfad gebunden, den `SpeechAnalyzer` laut Apple nicht benutzt. **Gesetzt wird er nicht** — die in Task 9.2 vorgesehene Gegenprobe ist am 2026-09-11 auf dem iPhone 16 Pro gelaufen: Phase 9 funktionierte vollständig ohne ihn. Die frühere Formulierung „gesetzt aus Vorsicht" beschrieb eine Absicht, die nie umgesetzt wurde, und ist hiermit in Phase 10 mit dem tatsächlichen Projektzustand zusammengeführt. Wortlaut, Belege und Konsequenzen in §7. | **Geschlossen**, Gegenprobe auf dem Gerät bestanden | entfällt |
...
| Q7 | Reicht die automatische SwiftData-Migration über die Projektlaufzeit? **Strategie in Phase 1 festgelegt** (kein `VersionedSchema`, Begründung als Kommentar an `CAppApp.makeModelContainer()`). Die Frage war bis Phase 10 **unbeantwortbar mangels Gelegenheit**: `CApp/Models/` hatte sich seit Phase 1 nicht geändert, es gab also nie etwas zu migrieren, und genau das stand hier — ohne die Annahme als bestätigt auszugeben. **Beantwortet am 2026-09-13 an der ersten echten Schemaänderung: ja, die leichtgewichtige Migration reicht für diesen Fall.** Phase 11 fügt das Modell `ReviewLog` hinzu und auf `Card` die Beziehung `reviews` (`.cascade`). Gemessen, nicht angenommen: `SchemaMigrationTests` schreibt einen Store mit dem **nachgebauten Phase-10-Schema** — dieselben Entitätsnamen `Card` und `Tag`, dieselben Attribute, **ohne** `reviews`, deklariert als eigene Klassen unter `Phase10Schema`, weil die App-Klassen die Erweiterung schon tragen — füllt ihn mit realistischen Daten (drei Karten, zwei Kategorien, Lernstände, `reviewCount`, `correctCount`, `lastReviewedAt`, manuell korrigiertes Pinyin, Mehrfachzuordnungen) und öffnet **dieselbe Datei** anschließend mit dem Phase-11-Schema. Ergebnis: Der Container öffnet ohne `SchemaMigrationPlan` und ohne `VersionedSchema`, alle drei Karten und beide Kategorien sind vorhanden, jeder geprüfte Wert ist unverändert, die Zuordnungen stehen, die neue Beziehung existiert und ist leer, und sie ist sofort benutzbar. Ein zweiter Test öffnet den migrierten Store **noch einmal**, weil eine Migration, die nur scheinbar funktioniert, erst beim nächsten Start auffiele. **Konsequenz: `project.pbxproj` und `CAppApp.makeModelContainer()` bleiben unverändert**, es wird weiterhin keine Migrationsinfrastruktur eingeführt. **Grenzen, die dazugehören:** (1) Belegt ist **diese** Änderung — additiv, ein neues Modell plus eine Beziehung. Für ein späteres Umbenennen oder Entfernen einer Property sagt der Test nichts, und dort wäre ein `SchemaMigrationPlan` weiterhin fällig. **Zweiter additiver Fall, am 2026-09-21 nachgemessen: neue Properties auf einem bestehenden Modell — ja, die leichtgewichtige Migration reicht auch dafür.** Phase 13 fügt `ReviewLog.suggestedStatusRaw: Int?` und `ReviewLog.suggestionDecisionRaw: String?` hinzu (beide optional, Default `nil`). Das ist ebenfalls additiv, aber ein *anderer* Fall als „neues Modell plus Beziehung", und der Phase-11-Test sagte darüber nichts — also wurde er nicht verallgemeinert, sondern der Fall einzeln gemessen: `SchemaMigrationTests.phase13PropertiesMigrateLightly` deklariert das **Phase-12-Schema** erneut (dieselben Entitätsnamen `Card`, `Tag`, `ReviewLog`, letzteres **ohne** die zwei Felder), schreibt damit eine Karte mit realer Historie — ein Eintrag mit einer der vier alten Selbsteinschätzungen, einer aus dem automatischen Pfad — und öffnet **dieselbe Datei** mit dem aktuellen Schema. Ergebnis: Der Container öffnet ohne `SchemaMigrationPlan`, jeder Wert ist unverändert, die historische Selbsteinschätzung bleibt lesbar, die beiden neuen Properties lesen auf jedem Alteintrag als `nil`, sie sind sofort beschreibbar, und ein **zweites** Öffnen bestätigt, was tatsächlich auf der Platte steht. **Präzisierung:** Seit der Nachlieferung enthält das aktuelle Schema zusätzlich `Card.classificationEvidenceResetAt`, dieser Test misst also Phase 12 → heute und isoliert die beiden `ReviewLog`-Properties nicht mehr allein. Das ist mehr als behauptet, nicht weniger; die Isolierung der `Card`-Property leistet der eigene Test unten. `project.pbxproj` und `CAppApp.makeModelContainer()` bleiben erneut unverändert. **Dritter additiver Fall, am selben Tag nachgemessen: eine neue Property auf `Card`.** Die manuelle Statuskorrektur braucht `Card.classificationEvidenceResetAt: Date?`; `SchemaMigrationTests.evidenceBoundaryMigratesLightly` baut dafür das **Phase-13-Schema** nach (`ReviewLog` mit den zwei Vorschlagsfeldern, `Card` ohne die Linie), schreibt eine Karte mit einem bereits abgelehnten Vorschlag, öffnet dieselbe Datei mit dem aktuellen Schema — jeder Wert unverändert, die neue Property als `nil`, also bleibt die ganze Historie zulässig —, benutzt sie sofort und öffnet ein zweites Mal zur Kontrolle. Drei Fälle, drei Messungen; aus zwei wird hier ausdrücklich kein Erfahrungswert gemacht. (2) Belegt ist ein **Testspeicher**, kein gewachsener privater Store auf dem iPhone. Der ist älter, größer und durch jede Version gegangen; seine Migration gehört ausdrücklich in die [finale Geräte- und Release-Abnahme](roadmap.md#finale-geräte--und-release-abnahme) und ist dort als Punkt geführt. | **Beantwortet für drei additive Fälle** — neues Modell + Beziehung (Phase 11), neue Properties auf `ReviewLog` und eine neue Property auf `Card` (beide Phase 13); **offen für nicht-additive Änderungen** | gering — `SchemaMigrationPlan` weiterhin nachrüstbar |
...
- [UITextInputMode](https://developer.apple.com/documentation/uikit/uitextinputmode) · [`activeInputModes`](https://developer.apple.com/documentation/uikit/uitextinputmode/activeinputmodes) · [`UIResponder.textInputMode`](https://developer.apple.com/documentation/uikit/uiresponder/textinputmode) · [`textInputContextIdentifier`](https://developer.apple.com/documentation/uikit/uiresponder/textinputcontextidentifier) · [`UIInputViewController`](https://developer.apple.com/documentation/uikit/uiinputviewcontroller) · [`openSettingsURLString`](https://developer.apple.com/documentation/uikit/uiapplication/opensettingsurlstring) · [Describing use of required reason API](https://developer.apple.com/documentation/bundleresources/describing-use-of-required-reason-api)
