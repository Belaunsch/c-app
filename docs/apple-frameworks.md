...
| Q7 | Reicht die automatische SwiftData-Migration über die Projektlaufzeit? **Strategie in Phase 1 festgelegt** (kein `VersionedSchema`, Begründung als Kommentar an `CAppApp.makeModelContainer()`). **Stand nach dem Phase-10-Rückblick am 2026-09-12 — vier Punkte, bewusst getrennt:** **(1) Seit Phase 1 gab es keine Schemaänderung.** Nachgesehen statt erinnert: `CApp/Models/` ist seit dem Commit `2e9b627` unverändert, im Verlauf wie im Arbeitsbaum; das Schema ist weiterhin `Schema([Card.self, Tag.self])`. Die Phasen 2 bis 10 haben nichts persistiert, was es vorher nicht gab — Pinyin, Tonsandhi, Lernlogik, TTS, Spracherkennung und die Einstellungen sind entweder berechnet, liegen in `UserDefaults` (`Preferences`) oder laufen ganz ohne Zustand. **(2) Daraus folgt: es gibt keine empirische Migrationsevidenz.** Die leichtgewichtige Migration hat bis heute nichts zu migrieren gehabt — kein Lauf, keine Messung, kein Gegenbeispiel, in beide Richtungen nichts. **(3) Es wird ausdrücklich nicht behauptet, die automatische Migration sei bewiesen.** Dass sie reicht, ist eine Annahme aus Phase 1 und bleibt als Annahme stehen. Ein grüner Testlauf sagt dazu nichts, ein bestandener Gerätetest auch nicht: Beide laufen auf einem Store, dessen Schema sich nie geändert hat. **(4) Konkreter nächster Entscheidungspunkt: die erste additive Schemaänderung, voraussichtlich `ReviewLog` in Phase 11.** Ein neues Modell ist der Fall, den SwiftData selbst erledigt; zu prüfen ist die **Beziehung** von `Card` dorthin, und zwar an echten Bestandsdaten auf dem Gerät. Ein Store aus dem Simulator beweist dabei nichts, weil er ohnehin neu angelegt wird. Erst dieser Lauf beantwortet die Frage; bis dahin bleibt sie offen. Das Phase-10-Akzeptanzkriterium ist aus diesem Grund am 2026-09-12 von „Q1–Q7 sind beantwortet“ auf „Q1–Q6 sind beantwortet; Q7 ist mit Begründung und konkretem Fälligkeitspunkt dokumentiert“ **präzisiert** worden — offen vermerkt in [roadmap.md](roadmap.md), nicht still überschrieben. | **Offen, mit Fälligkeitspunkt** — erste additive Schemaänderung, erwartet in Phase 11 | gering — `SchemaMigrationPlan` nachrüstbar, solange vorher nichts umbenannt oder entfernt wird |
